...
def compute(s: str) -> int:
    ...
    guards_max_minute = dict()

    for guard_id, sleeps in guards_sleeping.items():
        ...
        guards_max_minute[guard_id] = next(
            (minute, count) for minute, count in minute_counter.items()
            if count == max(minute_counter.values())
            )

    guard_id, (minute, count) = max(guards_max_minute.items(), key=lambda x: x[1][1])
